...
s.t. fit_all{j in 2..J}: sum{i in Client} x_ij[i,j] <= S_j[j] * is_open[j];
s.t. fit_first: sum{i in Client} x_ij[i,1] <= is_open[1] * (S_j[1] + S1_extra * is_extended);

minimize c: sum{j in WHouse} W_j[j]*is_open[j] +
sum{i in Client, j in WHouse} a_ij[i,j] * x_ij[i,j];
#is_extended * W1_extra * max(0,(sum{i in Client} x_ij[i,1] - S_j[1]));
